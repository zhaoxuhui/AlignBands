...
if __name__ == '__main__':
    if sys.argv.__len__() == 2 and sys.argv[1] == 'help':
        ...
        print("Read bands(tif file) and align with block mode."
        ...
        if flag == 'y':
            for it in range(exe_paths.__len__()):
                ...
                gb_dx, gb_dy = fun.getBandsOffsetWithNoStretchAuto(band_b_ori, band_g_ori, (it + 1))
                ...
                for i in range(blocks):
                    ...
                    if isStripeStretch:
                        if fun.isCloudMode:
                            kps_gb_g1, kps_gb_b1 = fun.alignRobust2BandsTIFWithStretchCloud(band_g_part1, band_b_part1,
                                                                                            it + 1,
                                                                                            i + 1)
                        else:
                            kps_gb_g1, kps_gb_b1 = fun.alignRobust2BandsTIFWithStretch(band_g_part1, band_b_part1,
                                                                                       it + 1,
                                                                                       i + 1)
                    else:
                        if fun.isCloudMode:
                            kps_gb_g1, kps_gb_b1 = fun.alignRobust2BandsTIFCloud(band_g_part1, band_b_part1, it + 1,
                                                                                 i + 1)
                        else:
                            kps_gb_g1, kps_gb_b1 = fun.alignRobust2BandsTIF(band_g_part1, band_b_part1, it + 1, i + 1)

                    # 当匹配点对小于3，无法构建仿射矩阵
                    if kps_gb_g1.__len__() < 3:
                        # 判断上一个条带是否成功生成仿射矩阵，如果是的话，使用上一个矩阵，否的话直接复制影像,part1部分结束
                    ...
                    # 匹配点对大于3，继续判断能否生成仿射矩阵以及生成的是否正确
                        ...
                        # 如果生成失败，判断上一个条带是否成功生成仿射矩阵，如果是的话，使用上一个矩阵，否的话直接复制影像，结束part1部分
                        if affine1 is None:
                            if affine_matrices_gb1.__len__() == 0:
                    ...
                    if isStripeStretch:
                        if fun.isCloudMode:
                            kps_gb_g2, kps_gb_b2 = fun.alignRobust2BandsTIFWithStretchCloud(band_g_part2, band_b_part2,
                                                                                            it + 1,
                                                                                            i + 1)
                        else:
                            kps_gb_g2, kps_gb_b2 = fun.alignRobust2BandsTIFWithStretch(band_g_part2, band_b_part2,
                                                                                       it + 1,
                                                                                       i + 1)
                    else:
                        if fun.isCloudMode:
                            kps_gb_g2, kps_gb_b2 = fun.alignRobust2BandsTIFCloud(band_g_part2, band_b_part2, it + 1,
                                                                                 i + 1)
                        else:
                            kps_gb_g2, kps_gb_b2 = fun.alignRobust2BandsTIF(band_g_part2, band_b_part2, it + 1, i + 1)

                    # 当匹配点对小于3，无法构建仿射矩阵
                    if kps_gb_g2.__len__() < 3:
                        # 判断上一个条带是否成功生成仿射矩阵，如果是的话，使用上一个矩阵，否的话直接复制影像,part2部分结束
                    ...
                    # 匹配点对大于3，继续判断能否生成仿射矩阵以及生成的是否正确
                        ...
                        # 如果生成失败，判断上一个条带是否成功生成仿射矩阵，如果是的话，使用上一个矩阵，否的话直接复制影像，结束part2部分
                        if affine2 is None:
                            if affine_matrices_gb2.__len__() == 0:
                                print("estimated affine matrix is none and no matrix to use, "
                                      "copy stripe image to dst stripe.")
