...
if __name__ == '__main__':
    if sys.argv.__len__() == 2 and sys.argv[1] == 'help':
        ...
        print("Read bands(png file) and align with global mode."
        ...
        print("\nUsage example:")
        print("AlignPNG_IMG.exe C:\\png C:\\pngout")
        os.system('pause')
    elif sys.argv.__len__() == 3:
        # 全图对准模式（png -> png）
        print('---Global mode for png files---')
        # 读取参数
        ...
        if flag == 'y':
            for it in range(exe_paths.__len__()):
                ...
                band_b = cv2.imread(path_b, cv2.IMREAD_GRAYSCALE)
                band_g = cv2.imread(path_g, cv2.IMREAD_GRAYSCALE)

                cost_time = []

                t1 = time.time()

                band_b_resample = band_b
                band_g_resample = band_g
                ...
                print("Save image...")
                # 保存jpg
                cv2.imwrite(out_path, resampled_band_b)
                print("Success.")
